...
$upquery = mysqli_query($con,"select * from `direct_billing` where id = '".$_REQUEST['upid']."'");
$upstatus_fetch = mysqli_fetch_array($upquery);
?>
<?php 
if(isset($_POST) && is_array($_POST) && $_POST['sendSMS'] == 'true'){
	function sendSms($mob='9726557717', $text='we will contact u soon'){
...
//	$ch = curl_init('http://mitechsolution.com/smsapi.aspx?username=harshvideo&password=harsh.video..&sender=HARSHV&to='.$mob.'&message='.$txt_msg.'&route=route3');
	$ch = curl_init('http://sms.mobileadz.in/api/push.json?apikey=5d6500c6cb45b&sender=SMSALT&mobileno='.$mob.'&text='.$txt_msg);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	$response = curl_exec($ch);	
}
$text = $_POST['title'].',
 Bill No: '.$_POST['invoice_no'].',
 Date:'.$_POST['date'].',
 Name: '.$_POST['name'].',
 Paid Amount:'.$_POST['amount_paid'].',
 Pre Pending Amount:'.$_POST['amonunt_due'];
sendSms($_POST['mobile'], $text);
}
...
$rowq= mysqli_fetch_array($resultq);
    ...
    <p id="address">M/s.&nbsp;<?php echo $upstatus_fetch['cust_name']; ?><br />
     
      Phone: (+91) <?php echo $upstatus_fetch['cust_mobile'] ; ?> </p>
    <table id="meta">
      <tr>
      ...
      <tr>
        ...
        <td><textarea id="date"><?php echo $upstatus_fetch['b_date']; ?></textarea></td>
      </tr>
      <tr>
  ...
  <table id="items">
    ...
    <?php			  $subtotal = 0;
		   					 ...
		   					 $sql_bd = "select * from `direct_billing_details` where `billing_id` = '".$upstatus_fetch['id']."' order by id asc";
  ...
  <form action="" method="post" id="print2">
    ...
    <div class="form-group">
      <label>Mobile</label>
      <input class="form-control" type="text" name="mobile" value="<?php echo $upstatus_fetch['cust_mobile'] ; ?>" />
    </div>
    <div class="form-group">
      <label>Title</label>
    ...
    <div class="form-group">
      <label>Date</label>
      <input class="form-control" type="text" name="date" value="<?php echo $upstatus_fetch['b_date']; ?>" />
    </div>
    <div class="form-group">
      <label>Name</label>
      <input class="form-control" type="text" name="name" value="<?php echo $upstatus_fetch['cust_name']; ?>" />
    </div>
    <div class="form-group">
  ...
  <form method="post" action="../mail_sent2.php" id="print3">
    <div class="form-group">
